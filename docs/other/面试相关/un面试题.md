...
beforeCreate、created、beforeMounted、render、mounted

- 4.简述每个周期具体适合哪些场景

    - beforeCreate：一般就是做一些权限判断之类的东西，控制路由，设置title等
    - created：第一次初始化数据，设置一些对数据的监听等；
    - beforeMounte：最后一次修改数据的机会
    - mounted：渲染完页面拿到数据，数据双向绑定。
    - beforeUpdate：修改数据前做的一些事情
    - updated：修改数据
    - beforeDestroy：去掉一些定时器，绑定元素等
    - destroy：做一些善后工作

- 5.created和mounted的区别

created是往实例上边挂载数据，绑定事件的，无dom结构，只有vm实例

mounted是在真实dom渲染后，对dom或者实例进行操作，有dom结构

- 6.vue获取数据在哪个周期函数

最好是在created获取，因为这个时候还没有虚拟dom不会增加一些消耗，
也可以在mounted中获取，这个适用于一些对dom操作的事情

在更新的时候在updated也可以获取数据

- 7.请详细说下你对vue生命周期的理解？

vue的生命周期就是实例从创建到销毁的一个过程，在这个过程中我们能够很好的对数据进行操作，而后把虚拟dom变成真实dom。
