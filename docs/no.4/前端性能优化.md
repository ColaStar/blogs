...
雅虎军规：核心：压缩合并与md5，html/css做hint
...
因为库文件不是频繁变动，做强缓（过期时间999年），业务文件（etag/last-modified/离线缓存）

websql可以存50m，但是读取速度缓慢，有可能比你的网络请求还长，他主要依赖硬件。

webpack打包一份key的js（存localstore的key，value）
